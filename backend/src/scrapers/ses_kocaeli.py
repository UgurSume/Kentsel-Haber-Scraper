"""
Scraper for Ses Kocaeli news site
"""
...
class SesKocaeliScraper(BaseScraper):
    """Scraper for seskocaeli.com (uses Selenium - Cloudflare protected)"""
    ...
    def scrape(self, days: int = 3) -> List[Dict]:
        """Scrape news articles from Ses Kocaeli"""
        articles = []
        ...
        try:
            ...
            for article_url in self.extract_candidate_urls(soup, max_links=80):
                # Ses Kocaeli article URLs follow /haber/{numeric-id}/slug pattern
    ...
    def parse_article(self, article_url: str) -> Optional[Dict]:
        """Parse a single article from Ses Kocaeli"""
        try:
